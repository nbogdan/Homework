...
/* Functia care descrie comportamentul receiver-ului petru task-ul 4 */
...
int Task4()
{
	...
	/* plasare bytes de corectie in pozitia buna */
	swapBytes(&t);
	/* aplicare corectie */
	correctHamming(&t);
	/* revenire */
	...
	while(1) {
		/* curatare msg */
		memset(&myPkg, 0, sizeof(pkgbonus));
		...
		if(find2(NAKArray, window, myPkg.packNum) >= 0)
		{
			/* salvam indicele ultimului pachet primit */
			last = current;
			/* retinem indicele pachetului curent*/
			current = find2(NAKArray, window, myPkg.packNum);
			...
			if(current - last > 1)
				do{
					...
					aux.len = strlen(aux.payload + 1);
					(*(pkgbonus*)aux.payload).packNum = NAKArray[last];
					(*(pkgbonus*)aux.payload).messageType = TYPE_NACK;
			...
			else if(last - current > 0)
			{
				...
				{	
					...
					aux.len = strlen(aux.payload + 1);
					(*(pkgbonus*)aux.payload).packNum = NAKArray[currentAux];
					(*(pkgbonus*)aux.payload).messageType = TYPE_NACK;
				...
				while(i < current){
					...
					aux.len = strlen(aux.payload + 1);
					(*(pkgbonus*)aux.payload).packNum = NAKArray[i];
					(*(pkgbonus*)aux.payload).messageType = TYPE_NACK;
			...
			mesajDuplicat = mesageDup2(&t);
			InsOrd2(&bufferMesaje, mesajDuplicat);
			...
			currentPack += EmptyBuffer2(&bufferMesaje, fdOut, currentPack);
			sprintf(aux.payload, "ACK Pachet numarul: (%d)", myPkg.packNum);
			aux.len = strlen(aux.payload + 1);
			(*(pkgbonus*)aux.payload).packNum = myPkg.packNum;
			(*(pkgbonus*)aux.payload).messageType = TYPE_ACK;
			...
			asteptat */
			i = find2(NAKArray, window, myPkg.packNum);
			NAKArray[i] = k++;
		}
		...
		un NACK, pentru unul din pachetele asteptate*/
			...
			if(NAKArray[i] < superior)
			{
				...
				aux.len = strlen(aux.payload + 1);
				(*(pkgbonus*)aux.payload).packNum = NAKArray[myAux];
				(*(pkgbonus*)aux.payload).messageType = TYPE_NACK;
	...
	sprintf((*(pkgbonus*)aux.payload).payload, "ACK Pachet final");
	aux.len = strlen(aux.payload + 1);
	(*(pkgbonus*)aux.payload).packNum = myPkg.packNum;	
	res = send_message(&aux);
